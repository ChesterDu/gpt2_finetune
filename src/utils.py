...
def load_data_knog(path, fname, enc, label):
    data = []
    print('loading %s/%s ......' % (path, fname))
    with open('%s/%s.txt' % (path, fname), encoding='utf-8') as f:
        for line in f:
            data.append({"st": enc.encode(line.strip()), "label": label})
    return data
...
def gen_batched_data_knog(data):
    max_story_len = max([len(item["st"]) for item in data]) + 1
    max_input_story_len = max_story_len
    story, story_length, label = [], [], []
    input_story, input_story_length = [], []

    for item in data:
        input_story.append(padding(item["st"], max_input_story_len))
        input_story_length.append(len(item["st"]) + 1)    
        story.append(item["st"])
