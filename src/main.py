...
from utils import FLAGS, enc, PAD_ID, hparams, \
    load_data, load_data_kg, load_data_knog, \
    gen_batched_data, gen_batched_data_from_kg, gen_batched_data_knog
...
def train(sess, dataset, is_train=True):
    ...
    while ed < len(dataset):
        ...
        elif FLAGS.data_name == "multi_kg":
            batch_data = gen_batched_data_knog(dataset[st:ed])
            input_feed = {
                context: batch_data["story"],
                context_length: batch_data["story_length"],
                label: batch_data["label"],
...
if FLAGS.data_name == "roc":
    ...
    data_train, data_dev, data_test = [], [], []
    # knog means positive data from kg, knog_neg means non-knowledge-graph data
    data_name = ["knog", "knog_neg", "knog_replace", "knog_repeat"]
    assert FLAGS.n_class == len(data_name)
    for _, name in enumerate(data_name):
        if "neg" in name:
            label = 1
            ...
            label = 0
        data = load_data_knog(FLAGS.data_dir, name, enc, label=label)
        data_segnum = len(data) / 20
        data_train.extend(data[:int(data_segnum*18)])
        data_dev.extend(data[int(data_segnum*18):int(data_segnum*19)])
        data_test.extend(data[int(data_segnum*19):])
...
with tf.Session(config=config) as sess:
    ...
    if FLAGS.is_train:
        ...
        while True:
            ...
            print("hejkrhqwlekhrldiujfhalkjsdhfalkjsdhfalkjsdhfaklsjdfhalkdfhakljdfhalkjdhflkajdhf")
